...
function G513Svg({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 380 140" preserveAspectRatio="xMidYMid meet" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {[0,1,2,3,4].map((i) => (
        <rect key={i} x="14" y={20 + i * 22} width="20" height="14" rx="2" fill="#1196ff" opacity="0.5"/>
      ))}
      {[...Array(14)].map((_, col) => (
        [...Array(4)].map((_, row) => (
          <rect key={`${col}-${row}`} x={50 + col * 22} y={20 + row * 26} width="18" height="20" rx="3" fill="#181825" stroke="#2a2a3e" strokeWidth="0.5"/>
        ))
      ))}
...
function GenericKeyboardSvg({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 340 120" preserveAspectRatio="xMidYMid meet" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {[...Array(12)].map((_, col) => (
        [...Array(4)].map((_, row) => (
          <rect key={`${col}-${row}`} x={18 + col * 26} y={18 + row * 22} width="22" height="16" rx="3" fill="#181825" stroke="#2a2a3e" strokeWidth="0.5"/>
        ))
      ))}
    </svg>
  );
...
export function DeviceImage({ modelId, deviceType, className }: Props) {
  ...
  if (deviceType === DeviceType.MOUSE) return <GenericMouseSvg className={className} />;
  if (deviceType === DeviceType.KEYBOARD) return <G513Svg className={className} />;
